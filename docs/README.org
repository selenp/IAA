#+HTML_HEAD:  <meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=0, maximum-scale=1, minimum-scale=1" />
#+HTML_HEAD: <style type="text/css">img{ max-width:98%; width: 800px }</style>

* 页面流程

** 主页面
 [[./main.png]]

** 领取设备
领取流程，共3个页面
*** 领取人 填写信息
 [[./b1.png]]

*** IT部门 填写设备信息
 [[./b2.png]]
 - 其中：部门、项目可选择，可输入，数据还可以在后台维护
*** 双方确认并签字，发邮件
 [[./b3.png]]

** 归还设备
 - 先查询领取记录，没有的话“新建”也可
 - 根据记录，可以查看从前的签名图片

** 查询页面
[[./r1.png]]

随后的归还流程，与“领取”基本一致

** 管理员

*** 登录
 [[./a1.png]]
 - 账户：zhaolei / zhaoleipass
*** 设备取还一览
 [[./d1.png]]

*** IT用户设备取还
 [[./t1.png]]

*** IT用户设备取还: 确定双方
 [[./t2.png]]

*** IT用户设备取还: 设备明细
 [[./t3.png]]

*** IT用户设备取还: 双方签字，发邮件
 [[./t4.png]]

*** 用户管理
 [[./u1.png]]

*** 数据字典管理
 [[./dic1.png]]

* <2018-05-15 Tue>需求
 - 门口 登记
 - 收发设备登记
 - 手机自助办理 填信息
 - 我们后台能看到

* <2018-05-11 Tue>
** 前台端Web模块
  - ipad上运行，浏览器页面程序
  - 主页面为“领”“还”大按钮页面

*** 领设备
  - 用户填写相关信息，并确认免责条款，签字，日期
  - IT填写，确认，并且签字

*** 还设备
  - 找到当时领设备的数据，进行归还
  - 可以新建归还

*** 手写签字
  - 参照 http://szimek.github.io/signature_pad/
  - https://jsfiddle.net/szimek/d6a78gwq/
** 管理员端Web模块
*** 设备进出一览
  - 可以增删改查
  - 可以进行excel的导入导出
*** CANCELLED 管理员一览
    CLOSED: [2018-05-11 Fri 14:21]
    :LOGBOOK:
    - State "CANCELLED"  from              [2018-05-11 Fri 14:21]
    :END:
  - 简易登录
  - 管理员增删改查，授权

** 开发
 - 后端开发用java，数据库为MS SQL，Web服务器为IIS
 - 前端页面为javascript + html的h5页面，页面能适应手机、pad、pc的浏览器

  IT人员收到通过别的系统得到相关指示，进行机器准备，随后通知相关用户，然后用户会前来领取设备，过一段时间后用户会来归还该设备


 归还：
 正常离职的
 更换的

* <2018-05-16 Wed>
 http://39.106.104.75:3022/#/delivery/main
** 发设备里面没有区分“IT领设备”和“用户领设备”，IT领设备不需要签那个声明，只需要签字就行
  - IT的情况下，省略第一步，而且签字背景图片需要不一样

** 签字那块，现在是在整个文档上都可以签，可不可以只限定在指定区域；到签字那块，就不能进行下一步了，满屏都是签字笔迹
  - 可以实现，稍微复杂一点儿而已。
  - 因为使用者看不出哪儿可以拖动，哪儿不能拖  --> 背景色区分一下即可
  - 借的签字图片、还的签字图片，都需要重新考虑背景格式

**  关于设备信息那块，比如序列号等，我们希望能够用扫描枪扫进去，可以设计成用扫描枪扫描，手动输入作为一个可选项; 换设备也是要用扫描枪扫描输入设备信息
 - 无线蓝牙条码枪就能完美解决这个问题。扫码枪连接上ipad，扫码的结果直接显示在当前焦点的输入框里面
 - 将来做成APP，能更好控制焦点 ----> 做成APP的话，就不需要扫码枪了，PAD直接能扫

** new
- 背景分开了
- 区分开，IT取还
- bugfix

* <2018-05-18 Fri>
** 基于当前演示的系统，需要做的改善
*** DONE 选择楼，楼层，座位号
    CLOSED: [2018-05-24 Thu 09:38]
    :LOGBOOK:
    - State "DONE"       from "NEXT"       [2018-05-24 Thu 09:38]
    :END:
*** DONE 台式机 / 笔记本  的要有区别
    CLOSED: [2018-05-24 Thu 10:05]
    :LOGBOOK:
    - State "DONE"       from "NEXT"       [2018-05-24 Thu 10:05]
    :END:
*** DONE 管理员后台能整理数据字典
    CLOSED: [2018-05-22 Tue 23:22]
*** DONE 修改成特定区域能签名 -> 可擦除
    CLOSED: [2018-05-22 Tue 23:22]
*** DONE IT的话，可以批量领取
    CLOSED: [2018-05-22 Tue 23:22]
*** DONE IT用户 与 普通用户，重合度已经不大了， 业务上分开
    CLOSED: [2018-05-22 Tue 23:22]
*** DONE IT当天处理不了的，还回去
    CLOSED: [2018-05-22 Tue 23:22]
*** DONE 新建“归还”
    CLOSED: [2018-05-24 Thu 09:38]
    :LOGBOOK:
    - State "DONE"       from "NEXT"       [2018-05-24 Thu 09:38]
    :END:
** DONE 用户管理
   CLOSED: [2018-05-24 Thu 10:52]
   :LOGBOOK:
   - State "DONE"       from "NEXT"       [2018-05-24 Thu 10:52]
   - State "NEXT"       from "DONE"       [2018-05-24 Thu 10:43]
   :END:
*** 系统管理员
 用户、角色、权限、其它数据整理等
*** asset管理员
按照预约，备货
*** IT
预约、领取、归还、签字
*** 普通用户
领取、归还、签字
** 用例 Case 1
 - faming 预约
 - IT准备 -》 根据数据准备设备
 - IT领取这批设备，签名，双方发邮件
 - 部分设备（因为某些事由）还回去 => 设备批量归还
 - excel报表下载，供各种计算

* 功能
** client app
 - 收发一览
 - 图片上载
 - excel

* API
** Account

  - api-login-account
 #+BEGIN_SRC sh
Content-Type: application/json
POST http://39.106.104.75:3050/account/api-login-account
Content-Type: application/json

{
"userid": "zhaolei",
"password": "zhaoleipass"
}
 #+END_SRC

  - currentUser
#+BEGIN_SRC sh
GET http://39.106.104.75:3050/account/currentUser
Authorization: Bearer SgxwdORcnzpCjfZ3eLqMwAQf7Qsc0w204bhWehIobvI=
#+END_SRC

** ADMIN
  - add
#+BEGIN_SRC sh
POST http://39.106.104.75:3050/admin/
Content-Type: application/json

{
"userid": "zhaolei",
"password": "zhaoleipass",
"fullname": "ZhaoLei",
"avatar": "https://gw.alipayobjects.com/zos/rmsportal/gaOngJwsRYRaVAuXXcmB.png"
}
#+END_SRC

  - api-login-account
 #+BEGIN_SRC sh
GET http://39.106.104.75:3050/admin/
 #+END_SRC

** DELIVERY
 - POST /delivery
 提交取还
#+BEGIN_SRC sh
POST http://39.106.104.75:3050/delivery/
Content-Type: application/json

{
"acPowerAdapter": true,
"assetTag": "8",
"bag": true,
"businessUnit": "5",
"eid": "1",
"fullname": "2",
"laptopModel": "0",
"location": "6",
"mouse": true,
"projectName": "3",
"remarks": "7",
"sapNumber": "3",
"securityCable": true,
"serialTag": "9",
"progress": "borrow"
}
#+END_SRC

 - GET /delivery/
 取还列表
#+BEGIN_SRC sh
GET http://39.106.104.75:3050/delivery/?eid=1
#GET http://39.106.104.75:3050/delivery/
#+END_SRC

 - GET /delivery/xlsx
 excle下载
#+BEGIN_SRC sh
GET http://39.106.104.75:3050/delivery/xlsx
# http://39.106.104.75:3022/images/poi-generated-file.xlsx
#+END_SRC

 - GET /delivery//:id
 某一次取还
#+BEGIN_SRC sh
GET http://39.106.104.75:3050/delivery/65
#+END_SRC

 - POST /delivery/signature
 上载签名图片
#+BEGIN_SRC sh
POST http://39.106.104.75:3050/delivery/4/borrow/signature
Content-Type: application/json

data:image/png;base64,
#+END_SRC
#+BEGIN_SRC sh
POST http://39.106.104.75:3050/delivery/4/return/signature
Content-Type: application/json

data:image/png;base64,
#+END_SRC


** TRANSFER
 - POST /transfer
 提交取还
#+BEGIN_SRC sh
POST http://39.106.104.75:3050/transfer-event/
Content-Type: application/json

{
"assetTags": "8,9,10",
"eid": "1",
"location": "6",
"remarks": "7",
"status": "borrow",
"ownerEid": "200"
}
#+END_SRC

 - GET /transfer-event/
 取还列表
#+BEGIN_SRC sh
GET http://39.106.104.75:3050/transfer-event/?eid=1
#GET http://39.106.104.75:3050/transfer-event/
#+END_SRC

 - GET /transfer-event/xlsx
 excle下载
#+BEGIN_SRC sh
GET http://39.106.104.75:3050/transfer-event/xlsx
# http://39.106.104.75:3022/images/poi-generated-file.xlsx
#+END_SRC

 - GET /transfer-event//:id
 某一次取还
#+BEGIN_SRC sh
GET http://39.106.104.75:3050/transfer-event/65
#+END_SRC

 - POST /transfer-event/signature
 上载签名图片
#+BEGIN_SRC sh
POST http://39.106.104.75:3050/transfer-event/4/borrow/signature
Content-Type: application/json

data:image/png;base64,
#+END_SRC
#+BEGIN_SRC sh
POST http://39.106.104.75:3050/transfer-event/4/return/signature
Content-Type: application/json

data:image/png;base64,
#+END_SRC

** Dictionary
  - 数据字典
#+BEGIN_SRC sh
GET http://39.106.104.75:3050/dictionary/_all
#+END_SRC
  - List
#+BEGIN_SRC sh
GET http://39.106.104.75:3050/dictionary/
#+END_SRC

 - 删除一个
#+BEGIN_SRC sh
DELETE http://39.106.104.75:3050/dictionary/5
#+END_SRC

* Deploy
** 同步
#+BEGIN_SRC sh
gradle build
scp build/libs/gs-accessing-data-h2-0.1.0524.jar  yidalize_no1:/usr/share/nginx/html/yida-lize-file/
java -jar build/libs/gs-accessing-data-h2-0.1.0522.jar

rm -rf dist/; npm run build;  rsync -avz /works/projects/accenture/equipment/antd-web/dist/ yidalize_no1:/usr/share/nginx/html/yida-lize-file/

http://39.106.104.75:3022/#/delivery/main
#+END_SRC
